...
class AdmUsuarioController extends Controller
{
	...
	{
		$model=new AdmUsuario;
		$data = AdmUsuario::model()->findAll();
		$label_colunas = $this->get_label_colunas($model);
		$DropDownList = null;
		...
		if(isset($_POST['AdmUsuario']))
		{
			$model->attributes=$_POST['AdmUsuario'];
	...
	{
		$dataProvider=new CActiveDataProvider('AdmUsuario');
	...
	{
		$data= AdmUsuario::model()->findAll(); 
		$model=new AdmUsuario('search');
		$model->unsetAttributes();  // clear any default values
		$label_colunas = $this->get_label_colunas($model);
		
		if(isset($_GET['AdmUsuario']))
			$model->attributes=$_GET['AdmUsuario'];
	 ...
	 * @return AdmUsuario the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=AdmUsuario::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param AdmUsuario $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='adm-usuario-form')
		{
	 ...
	 * @param  $data Todos os dados da tabela do model AdmUsuario.
	 */
	protected function getFkDropDownList($data){
		...
		foreach ($relations as $relation){
			...
			$fdata  = AdmUsuario::model($fTable)->findAll();
			if(count($fdata)>0){
